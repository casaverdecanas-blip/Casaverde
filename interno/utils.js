...
//
//  CAMBIOS v4.1:
//  - [NUEVO] sincronizarDesdeGCal() — función unificada para
//    sincronización Airbnb via Google Calendar. Reemplaza las
//    implementaciones duplicadas en dashboard.html y calendario.html.
//    NO genera tareas de limpieza — eso lo hace el admin al
//    completar y confirmar la reserva.
// ============================================================
...
//
//  Función unificada — reemplaza las implementaciones duplicadas
//  en dashboard.html y calendario.html.
//
//  Lee Google Calendar de cada cabaña via API pública de Google
//  (no requiere autenticación del usuario, solo API key).
//  Crea reservas nuevas y detecta cancelaciones.
//
//  NO genera tareas de limpieza — eso lo hace el admin al
//  completar y confirmar la reserva, cuando ya tiene los datos
//  reales del huésped (cantidad, horarios, requerimientos).
//
//  @param googleApiKey  {string}  API key de Google Calendar
//  @param cabanasCache  {Array}   Cabañas con google_calendar_id
//  @returns {Promise<{creadas, omitidas, canceladas, error?}>}
//
async function sincronizarDesdeGCal(googleApiKey, cabanasCache) {
    ...
    // Leer eventos de Google Calendar para una cabaña
    ...
    // Verificar que haya cabañas con Calendar ID configurado
    const cabañasConCal = (cabanasCache || []).filter(
        c => c.google_calendar_id && c.google_calendar_id.includes('@')
    );
    if (!cabañasConCal.length) {
        return { creadas: 0, omitidas: 0, canceladas: 0,
            error: 'Sin cabañas con Calendar ID configurado' };
    }

    // Leer todos los calendarios en paralelo
    const resultados   = await Promise.all(cabañasConCal.map(c => leerGCal(c.google_calendar_id, c.id)));
    const todosEventos = resultados.flat();

    // Leer reservas Airbnb existentes en Firestore
    ...
    // Detectar cancelaciones: estaban en Firestore pero ya no están en GCal
    let canceladas = 0;
    ...
    // Crear reservas para eventos nuevos
    // Sin tarea de limpieza — el admin la genera al completar la reserva
    ...
    for (const ev of todosEventos) {
        ...
        const reservaData = {
            ...
            totalBRL: 0,
            notas:    'Importado de Airbnb via Google Calendar. Título: ' + ev.titulo,
    ...
    // Sincronizar disponibilidad pública para las nuevas reservas
...
async function getHistorialTareas(tareaId) {
    try {
        ...
        if (registros.length === 0) {
            return {
                totalVeces: 0, ultimaVez: null, porUsuario: [],
                resumen: { totalHoras: 0, totalMonto: 0, colaboradoresUnicos: 0 }
            };
        }
        ...
        for (const reg of registros) {
            ...
            for (const col of colaboradores) {
                ...
                if (!porUid[col.uid]) {
                    porUid[col.uid] = { nombre: col.nombre, veces: 0, horasTotal: 0, montoTotal: 0, ultimaVez: null };
                }
        ...
        const porUsuarioArray = Object.values(porUid).sort((a, b) => b.veces - a.veces);
