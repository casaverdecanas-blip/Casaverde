...
const CABIN_ID = 'cabin-2'; // Identificador único para Cabaña 2
...
function addPhotoUrl() {
    const url = prompt("Pega la URL de la imagen para Cabaña 2:");
    if(url) db.ref(`fotos/${CABIN_ID}`).push(url);
}

function deletePhoto(key) { if(confirm("¿Eliminar foto de Cabaña 2?")) db.ref(`fotos/${CABIN_ID}/${key}`).remove(); }
...
function openAdminModal() {
    const p = prompt("Clave de Cabaña 2:");
    if (p === "Casaverde-199") {
        editMode = !editMode;
